...
var assets = {
  ...
  "animations":{
	...
	"player": {
		...
		"score-stone": {
			"strip": "../shared/images/score-stone.png",
			"frames": 1,
			"msPerFrame": 50
		},
  },
...
game.scenes.add("title", new Splat.Scene(canvas, function() {
	...
	var scoreStoneSprite = game.animations.get("score-stone");
	this.scoreStone = new Splat.AnimatedEntity(center("x", scoreStoneSprite) ,canvas.height - scoreStoneSprite.height , scoreStoneSprite.width, scoreStoneSprite.height, scoreStoneSprite, 0, 0);
	...
	this.timers.makeFood = new Splat.Timer(undefined, 1000, function(){
		makeFood(scene.food, scene.foodSprite);
		this.reset();
		this.start();
	});
	this.timers.makeFood.start();
	...
	this.foodSprite.move(time);

	for (var i = 0; i < this.food.length; i++){
		if(this.player.collides(this.food[i])){
	...
	// draw
  	...
  	this.scoreStone.draw(context);
	context.fillStyle = "#AAA477";
...
function makeFood(array, sprite) {
	...
	foodEntity.angle = randomRange(0, 360);
